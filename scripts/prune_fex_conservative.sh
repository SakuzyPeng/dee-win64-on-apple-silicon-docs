...
BACKUP_BASE="${BACKUP_BASE:-$ROOT_DIR/tmp_fex_rootfs/ConservativePruneBackup}"

usage() {
  cat <<'EOF'
Usage:
  scripts/prune_fex_conservative.sh --status
  scripts/prune_fex_conservative.sh --apply
  scripts/prune_fex_conservative.sh --rollback [--backup-dir DIR]

Description:
  Conservative prune for FEX rootfs:
  - apt/swcatalog cache
  - renderdoc/gfxrecon related tools and libs

  --apply moves files/dirs to backup for reversible rollback.

Environment:
  ROOTFS_DIR   Rootfs path (default: tmp_fex_rootfs/RootFS/Ubuntu_24_04)
  BACKUP_BASE  Backup base dir (default: tmp_fex_rootfs/ConservativePruneBackup)
EOF
}
...
collect_targets() {
  add_if_exists() {
    rel="$1"
    [[ -e "$ROOTFS_DIR/$rel" ]] || return 0
    echo "$rel"
  }

  add_if_exists "var/cache/apt"
  add_if_exists "var/cache/swcatalog"

  add_if_exists "usr/bin/qrenderdoc"
  add_if_exists "usr/bin/renderdoccmd"
  add_if_exists "usr/bin/spirv2dxil"

  for abs in "$ROOTFS_DIR"/usr/bin/gfxrecon-*; do
    [[ -e "$abs" ]] || continue
    rel="${abs#"$ROOTFS_DIR/"}"
    echo "$rel"
  done

  add_if_exists "usr/lib/x86_64-linux-gnu/librenderdoc.so"
  add_if_exists "usr/lib/x86_64-linux-gnu/libspirv_to_dxil.a"
  add_if_exists "usr/lib/x86_64-linux-gnu/libspirv_to_dxil.so"
  add_if_exists "usr/lib/x86_64-linux-gnu/libVkLayer_gfxreconstruct.so"
}
...
apply_mode() {
  ...
  if [[ "$moved" -eq 0 ]]; then
    echo "No conservative targets found. Nothing changed."
    rm -f "$manifest"
