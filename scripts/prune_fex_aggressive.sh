...
BACKUP_BASE="${BACKUP_BASE:-$ROOT_DIR/tmp_fex_rootfs/AggressivePruneBackup}"

usage() {
  cat <<'EOF'
Usage:
  scripts/prune_fex_aggressive.sh --status
  scripts/prune_fex_aggressive.sh --apply
  scripts/prune_fex_aggressive.sh --rollback [--backup-dir DIR]

Description:
  Aggressive prune for FEX rootfs (graphics stack):
  - LLVM/mesa/vulkan large libs
  - dri drivers
  - mesa demo/share metadata

  --apply moves targets to backup for reversible rollback.

Environment:
  ROOTFS_DIR   Rootfs path (default: tmp_fex_rootfs/RootFS/Ubuntu_24_04)
  BACKUP_BASE  Backup base dir (default: tmp_fex_rootfs/AggressivePruneBackup)
EOF
}
...
collect_targets() {
  ...
  add_if_exists "usr/lib/x86_64-linux-gnu/libLLVM.so.20.1"
  add_if_exists "usr/lib/x86_64-linux-gnu/libgallium-25.3.2 - kisak-mesa PPA.so"
  add_if_exists "usr/lib/x86_64-linux-gnu/dri"
  add_if_exists "usr/share/mesa-demos"
  add_if_exists "usr/share/vulkan"

  for abs in "$ROOTFS_DIR"/usr/lib/x86_64-linux-gnu/libvulkan*.so*; do
    [[ -e "$abs" ]] || continue
    rel="${abs#"$ROOTFS_DIR/"}"
    echo "$rel"
  done
...
apply_mode() {
  ...
  if [[ "$moved" -eq 0 ]]; then
    echo "No aggressive targets found. Nothing changed."
    rm -f "$manifest"
